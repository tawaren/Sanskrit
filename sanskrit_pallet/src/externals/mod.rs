...
//12
pub static mut SYS_HASH: Option<Hash> = None;

pub static mut SYS_MODS: [fn(Hash)->();16] = [
            |h|{unsafe{EXT_MAP.insert(h, i8::EXT_I8)};},        //0
            |h|{unsafe{EXT_MAP.insert(h, i16::EXT_I16)};},      //1
            |h|{unsafe{EXT_MAP.insert(h, i32::EXT_I32)};},      //2
            |h|{unsafe{EXT_MAP.insert(h, i64::EXT_I64)};},      //3
            |h|{unsafe{EXT_MAP.insert(h, i128::EXT_I128)};},    //4
            |h|{unsafe{EXT_MAP.insert(h, u8::EXT_U8)};},        //5
            |h|{unsafe{EXT_MAP.insert(h, u16::EXT_U16)};},      //6
            |h|{unsafe{EXT_MAP.insert(h, u32::EXT_U32)};},      //7
            |h|{unsafe{EXT_MAP.insert(h, u64::EXT_U64)};},      //8
            |h|{unsafe{EXT_MAP.insert(h, u128::EXT_U128)};},    //9
            |h|{unsafe{EXT_MAP.insert(h, data::EXT_DATA)};},    //10
            |h|{unsafe{EXT_MAP.insert(h, ids::EXT_IDS)};},      //11
            |h|{unsafe{SYS_HASH = Some(h)};},                     //12
...
impl CompilationExternals for ServerExternals {
    fn compile_call<'b, 'h>(module: &ModuleLink, fun_idx: u8, params: SlicePtr<'b, ValueRef>, caller: &[u8; 20], alloc: &'b HeapArena<'h>) -> Result<CompilationResult<'b>> {
        match unsafe{EXT_MAP.get(&module.to_hash())} {
...
impl<'c> SystemDataManager<BundleWithHash<'c>> for ServerSystemDataManager {
    ...
    fn providable_size(typ: Ptr<RuntimeType>) -> Result<u32> {
        match *typ {
            RuntimeType::Custom { module, offset, .. } if Some(module) == unsafe{SYS_HASH} && offset == 1 => {
                Ok((Hash::SIZE + 4*Entry::SIZE) as u32)
            }
            RuntimeType::Custom { module, offset, .. } if Some(module) == unsafe{SYS_HASH} && offset == 2 => {
                Ok((Hash::SIZE + 2*Entry::SIZE) as u32)
            }
            RuntimeType::Custom { module, offset, .. } if Some(module) == unsafe{SYS_HASH} && offset == 3 => {
    ...
    fn is_chain_value(typ: Ptr<RuntimeType>) -> bool {
        match *typ {
            RuntimeType::Custom { module, offset, .. } if Some(module) == unsafe{SYS_HASH} && offset == 0 => true,
            _ => false
        }
    }

    //This means we can only provide 1 value per Txt
    fn provided_value_key(typ: Ptr<RuntimeType>, section_no:u8,  txt_no:u8, _p_num:u8) -> Option<Vec<u8>> {
        match *typ {
            //This means we can only provide 1 value per Txt
            RuntimeType::Custom { module, offset, .. } if Some(module) == unsafe{SYS_HASH} && offset == 2 => {
    ...
    fn create_provided_value<'a, 'h>(bundle: &BundleWithHash, typ: Ptr<RuntimeType>, alloc: &'a VirtualHeapArena<'h>, block_no: u64, section_no:u8,  txt_no:u8, _p_num:u8) -> Result<Entry<'a>> {
        match *typ {
            RuntimeType::Custom { module, offset, .. } if Some(module) == unsafe{SYS_HASH} && offset == 1 => {
