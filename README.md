...
### Checked Exceptions
With this method each function declares its exceptional behaviour and a caller of such a function is either required to declare the same exceptional behaviour themself or define a handler for the exception when calling the function. An exception can contain values that are made avaiable to the handler. A function can raise its declared exceptions which requires to provide the contained values. This is generally more complicated to implement as it adds a new Dimension to the static verification during compilation. This system could be combined with Transaction if done carefully.
