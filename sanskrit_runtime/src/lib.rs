...
pub fn execute<S:Store>(store:&S, txt_data:&[u8], block_no:u64, heap:&Heap) -> Result<Hash> {
    ...
    let full_hash = HashingDomain::Transaction.hash(&txt_data);
    //hash over everything except signatures
    let txt_hash = HashingDomain::Transaction.hash(&txt_data[..sigs_start]);
    //hash over the code only
    let code_hash = HashingDomain::Transaction.hash(&txt_data[code_start..sigs_start]);
