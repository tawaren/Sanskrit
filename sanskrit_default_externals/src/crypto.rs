...
pub fn plain_hash<I:ExecutionInterface>(inter:&mut I, kind:Kind, val:ValueRef, tail:bool) {
...
pub fn join_hash<I:ExecutionInterface>(inter:&mut I, val1:ValueRef, val2:ValueRef, domain:HashingDomain, tail:bool) {
...
pub fn ecdsa_verify<I:ExecutionInterface>(inter:&mut I, msg:ValueRef, pk:ValueRef, sig:ValueRef, tail:bool) {
    let msg_data = inter.get(msg);
    let pk_data = inter.get(pk);
    let sig_data = inter.get(sig);

    assert!(pk_data.as_data().len() == 32);
    assert!(sig_data.as_data().len() == 64);
