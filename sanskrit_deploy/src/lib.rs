...
pub fn validate_stored_module<S:StateManager>(provider:S, link:FastModuleLink, system_mode_allowed:bool) -> Result<()>{
    //validates the input
    validate::validate(provider, link, system_mode_allowed)
}

//Processes a function used by compiler to check top level transactions
pub fn validate_unparsed_function<S:StateManager>(provider:S, data:&[u8]) -> Result<Hash>{
