...
pub enum OpCode {
    ...
    /*1*/Let(Exp),                                                               //A Subscope that computes some values and returns them (intermidiary values are removed)
    ...
    /*9*/InspectUnpack(ValueRef, PermRef),                                       //Inspects and locks (until exp end) a value to produce its fields (single Ctr only) (Needs Inspect Cap)
    /*10*/CopyField(ValueRef, PermRef, u8),                                      //<-- requires Inspect & Field to have Copy
    /*11*/Field(ValueRef, PermRef, u8),                                          //<-- requires Consume & others to have Drop
    /*12*/CopySwitch(ValueRef, PermRef, Vec<Exp>),                               //Branches on a type that has multiple ctrs where each branch corresponds to 1 Ctr (Does an implicit CopyUnpack)
    /*13*/Switch(ValueRef, PermRef, Vec<Exp>),                                   //Branches on a type that has multiple ctrs where each branch corresponds to 1 Ctr (Does an implicit Unpack)
    /*14*/InspectSwitch(ValueRef, PermRef, Vec<Exp>),                            //Branches on a type that has multiple ctrs where each branch corresponds to 1 Ctr (Does an a special read only unpack)
    /*15*/CopyPack(PermRef, Tag, Vec<ValueRef>),                                 //Generates a value for a multi ctr mutli field type by coping the inputs (requieres copy cap for them)
    /*16*/Pack(PermRef, Tag, Vec<ValueRef>),                                     //Generates a value for a multi ctr mutli field type
    /*17*/Invoke(PermRef, Vec<ValueRef>),                                        //Invokes a Callable
    /*18*/TryInvoke(PermRef, Vec<(bool, ValueRef)>, Exp, Exp),                   //Invokes a Callable and handles failures
    /*19*/InvokeSig(ValueRef, PermRef, Vec<ValueRef>),                           //Invokes a Signature Value
    /*20*/TryInvokeSig(ValueRef, PermRef, Vec<(bool, ValueRef)>, Exp, Exp),      //Invokes a Signature Value
    /*21*/RepeatedInvoke(u8, PermRef, Vec<ValueRef>, u8, u8),
    /*22*/RepeatedTryInvoke(u8, PermRef, Vec<(bool, ValueRef)>, u8, u8, Exp, Exp),
    /*23*/Project(TypeRef, ValueRef),                                            //Produces the Image of the input without consuming it
    /*24*/UnProject(TypeRef, ValueRef),                                          //Removes a layer from a nested Projection: If the inner value is Primitive
    /*25*/RollBack(Vec<ValueRef>, Vec<TypeRef>)                                  //Aborts the current Transaction, can consume and produce values to please the type checker
}
