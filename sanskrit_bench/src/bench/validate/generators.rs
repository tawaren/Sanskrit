...
pub fn generate(n:u8, reuse:bool, overfill:bool, body:bool) -> Vec<Vec<u8>> {
    ...
    if !reuse {
        ...
        let module_data = generate_function_module(n,0, reuse, overfill);
        hashes.push(store_hash(&[&module_data]));
        modules.push(module_data)
    }
...
fn generate_import_function(n:u8) -> FunctionComponent {
    ...
    let mut codes = Vec::with_capacity((n+1) as usize);
    let mut ret_vals = Vec::with_capacity(n as usize);
    ...
    for offset in 0..n {
        ...
        ret_vals.push(ValueRef(offset as u16))
    }
    ret_vals.reverse();
    codes.push(OpCode::Return(ret_vals));
