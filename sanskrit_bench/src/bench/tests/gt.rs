...
pub struct GtTest(usize,Kind,isize);
impl Op for GtTest {
    fn get_kind(&self) -> Kind { self.1 }
    fn get_params(&self) -> usize { 2 }
    fn get_base_num(&self) -> isize { self.2 }
    fn get_repeats(&self) -> usize { self.0 }
    fn build_opcode<'b>(&self, iter: usize , _:&'b VirtualHeapArena)-> OpCode<'b> { OpCode::Gt(self.1, ValueRef(iter as u16), ValueRef((iter + 1) as u16)) }
}

lazy_static! {
    pub static ref MEASURE_CACHE: Mutex<BTreeMap<OpTest<GtTest>, u128>> = Mutex::new(BTreeMap::new());
}

fn measure(test:OpTest<GtTest>, loops:usize) -> u128 {
...
fn test(rep:usize, kind:Kind, num:isize) -> OpTest<GtTest> { OpTest(GtTest(rep, kind, num))}
