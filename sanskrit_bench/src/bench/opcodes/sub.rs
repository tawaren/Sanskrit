...
//Result: 27-29 -> roundup to: 30
#[derive(Copy, Clone, Eq, PartialEq, Ord, PartialOrd)]
pub struct SubTest(usize,Kind,isize);
impl Op for SubTest {
    fn get_kind(&self) -> Kind { self.1 }
    fn get_params(&self) -> usize { 2 }
    fn get_base_num(&self) -> isize { self.2 }
    fn get_repeats(&self) -> usize { self.0 }
    fn build_opcode<'b>(&self, iter: usize, _:&'b VirtualHeapArena) -> OpCode<'b> { OpCode::Sub(self.1, ValueRef(iter as u16), ValueRef((iter + 1) as u16)) }
}

lazy_static! {
    pub static ref MEASURE_CACHE: Mutex<BTreeMap<OpTest<SubTest>, u128>> = Mutex::new(BTreeMap::new());
}

fn measure(test:OpTest<SubTest>, loops:usize) -> u128 {
...
fn test(rep:usize, kind:Kind, num:isize) -> OpTest<SubTest> { OpTest(SubTest(rep, kind, num))}
