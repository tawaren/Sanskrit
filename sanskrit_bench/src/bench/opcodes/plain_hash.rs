...
//Result: 300 + (25*Bytes)/20 (rounded up - actually 21-23/20  Byte)
// Primitives all have less than 20 Byte so they are 300
// Note: up to 100 we have constant 300 but that would be an akward formula
#[derive(Copy, Clone, Eq, PartialEq, Ord, PartialOrd)]
pub struct PlainHashTest(usize,Kind,isize);
impl Op for PlainHashTest {
    ...
    fn build_opcode<'b>(&self, iter: usize, alloc:&'b VirtualHeapArena)-> OpCode<'b> {
        OpCode::TypedSysInvoke(0, self.1, alloc.copy_alloc_slice(&[ValueRef(iter as u16)]).unwrap())
    }
}

lazy_static! {
    pub static ref MEASURE_CACHE: Mutex<BTreeMap<OpTest<PlainHashTest>, u128>> = Mutex::new(BTreeMap::new());
}

fn measure(test:OpTest<PlainHashTest>, loops:usize) -> u128 {
...
fn test(rep:usize, kind:Kind, num:isize) -> OpTest<PlainHashTest> {OpTest(PlainHashTest(rep,kind,num))}

pub fn measure_gas(loops:usize) {
    let bin_op = "PlainHash";
