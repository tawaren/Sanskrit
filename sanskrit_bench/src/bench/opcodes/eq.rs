...
//Data: 30 + (2*Bytes)/5  /Note: this is so cheap as memcmp probably is used under the hood
#[derive(Copy, Clone, Eq, PartialEq, Ord, PartialOrd)]
pub struct EqTest(usize,Kind,isize);
impl Op for EqTest {
    fn get_kind(&self) -> Kind { self.1 }
    fn get_params(&self) -> usize { 2 }
    fn get_base_num(&self) -> isize { self.2 }
    fn get_repeats(&self) -> usize { self.0 }
    fn build_opcode<'b>(&self, iter: usize , _:&'b VirtualHeapArena)-> OpCode<'b> { OpCode::Eq(self.1, ValueRef(iter as u16), ValueRef((iter + 1) as u16)) }
}

lazy_static! {
    pub static ref MEASURE_CACHE: Mutex<BTreeMap<OpTest<EqTest>, u128>> = Mutex::new(BTreeMap::new());
}

fn measure(test:OpTest<EqTest>, loops:usize) -> u128 {
...
fn test(rep:usize, kind:Kind, num:isize) -> OpTest<EqTest> { OpTest(EqTest(rep, kind, num))}
