...
//Result: 26-29 -> round to: 30
#[derive(Copy, Clone, Eq, PartialEq, Ord, PartialOrd)]
pub struct LteTest(usize,Kind,isize);
impl Op for LteTest {
    fn get_kind(&self) -> Kind { self.1 }
    fn get_params(&self) -> usize { 2 }
    fn get_base_num(&self) -> isize { self.2 }
    fn get_repeats(&self) -> usize { self.0 }
    fn build_opcode<'b>(&self, iter: usize , _:&'b VirtualHeapArena)-> OpCode<'b> { OpCode::Lte(self.1, ValueRef(iter as u16), ValueRef((iter + 1) as u16)) }
}

lazy_static! {
    pub static ref MEASURE_CACHE: Mutex<BTreeMap<OpTest<LteTest>, u128>> = Mutex::new(BTreeMap::new());
}

fn measure(test:OpTest<LteTest>, loops:usize) -> u128 {
...
fn test(rep:usize, kind:Kind, num:isize) -> OpTest<LteTest> { OpTest(LteTest(rep, kind, num))}
