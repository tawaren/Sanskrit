...
//Result: 22-23 -> roundup to: 25
// Data: 35 + (8*Bytes)/5
#[derive(Copy, Clone, Eq, PartialEq, Ord, PartialOrd)]
pub struct NotTest(usize,Kind,isize);
impl Op for NotTest {
    fn get_kind(&self) -> Kind { self.1 }
    fn get_params(&self) -> usize { 1 }
    fn get_base_num(&self) -> isize { self.2 }
    fn get_repeats(&self) -> usize { self.0 }
    fn build_opcode<'b>(&self, iter: usize, _:&'b VirtualHeapArena)-> OpCode<'b> { OpCode::Not(self.1, ValueRef(iter as u16)) }
}

lazy_static! {
    pub static ref MEASURE_CACHE: Mutex<BTreeMap<OpTest<NotTest>, u128>> = Mutex::new(BTreeMap::new());
}

fn measure(test:OpTest<NotTest>, loops:usize) -> u128 {
...
fn test(rep:usize, kind:Kind, num:isize) -> OpTest<NotTest> {OpTest(NotTest(rep,kind,num))}
...
pub fn measure_gas(loops:usize) {
    ...
    for (kind, name, num) in &[
        ...
        println!("{}_{} - {}", un_op, name, measure(test(1000, *kind, *num), loops)/1000)
    }
    let base = measure(test(1000,Kind::Data, 0), loops) as i128;
    println!("{}Data{} - {}", un_op, 0, base/1000);
    ...
    for i in &trials {
        ...
        println!("{}Data{} - {}", un_op, i, res/1000);
        inter.add_measure(res, *i);
    }

    println!("{}Data - {} + {}*bytes", un_op, base/1000, inter.eval()/1000.0)
}
