...
//Result: 27-28 -> roundup to: 30
// Exception i128 uses 350 - 390 <- why???
#[derive(Copy, Clone, Eq, PartialEq, Ord, PartialOrd)]
pub struct MulTest(usize,Kind,isize);
impl Op for MulTest {
    fn get_kind(&self) -> Kind { self.1 }
    fn get_params(&self) -> usize { 2 }
    fn get_base_num(&self) -> isize { self.2 }
    fn get_repeats(&self) -> usize { self.0 }
    fn build_opcode<'b>(&self, iter: usize, _:&'b VirtualHeapArena) -> OpCode<'b> { OpCode::Mul(self.1, ValueRef(iter as u16), ValueRef((iter + 1) as u16)) }
}

lazy_static! {
    pub static ref MEASURE_CACHE: Mutex<BTreeMap<OpTest<MulTest>, u128>> = Mutex::new(BTreeMap::new());
}

fn measure(test:OpTest<MulTest>, loops:usize) -> u128 {
...
fn test(rep:usize, kind:Kind, num:isize) -> OpTest<MulTest> { OpTest(MulTest(rep, kind, num))}
