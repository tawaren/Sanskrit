...
pub mod gas {
    ...
    use sanskrit_core::model::DataImpl;
    /*
    ...
    fn traversing_statistics<S:Store>(typ:&Crc<ResolvedType>, context:&Context<S>, node_fix:u64,  leaf_static:u64, leaf_per_byte:u64, prim_fact:u64) -> Result<u64> {
        Ok(match **typ {
            ResolvedType::Data { ref module, offset, ref applies, ..} => {
                ...
                //todo: the traversing_statistics will vanish so
                match adt.body {
    ...
    */
    ...
    pub fn try(catches:usize) -> u64 {
        16 + (catches as u64) //catches is a educated guess as the measurements where flawed*/
    }
}
